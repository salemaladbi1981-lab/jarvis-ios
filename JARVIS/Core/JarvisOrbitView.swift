...
struct AgentChip: View {
    ...
    var body: some View {
        Text(agent.name)
            ...
            .padding(.vertical, 6)
            .background(Capsule().fill(JarvisColor.bg_1.opacity(isActive ? 0.82 : 0.45)))
            .overlay(Capsule().stroke(isActive ? JarvisColor.primary_blue : JarvisColor.primary_blue.opacity(0.16), lineWidth: 1))
