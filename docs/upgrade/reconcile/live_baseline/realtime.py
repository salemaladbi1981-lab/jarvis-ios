...
Stability fixes (Sep 2026, UPG-1):
...
- No bare `except: pass` — structured trace logging (no personal content / args / tokens / audio).
"""
...
def _trace(tag, msg):
    # أحداث وأزمنة فقط — لا نصوص محادثة ولا args ولا صوت ولا توكنات ولا إحداثيات دقيقة.
...
async def openai_realtime_proxy(client_ws, session_config: dict):
    ...
    async with websockets.connect(url, additional_headers=headers) as upstream:
        ...
        def dispatch_tool(name, args):
            # توزيع التنفيذ حسب البادئة — لا نص args داخل السجل
        ...
        async def execute_tool_and_respond(name, args, call_id, gen, specialist):
            ...
            if name == "youtube_play" and output.get("ok"):
                ...
                _trace("handoff", "youtube_play -> ok")
