...
# Registry بحسابين: personal + work (نفس message id "m1" في كليهما عمداً لاختبار عدم الاختلاط)
reg = fake_registry([
...
# 2) حسابان → unified summary صحيح (كل رسالة تحمل account_id + account)
...
# 6) محاولة send من حساب مختلف → blocked (account_mismatch)
pending2 = {}
...
# 7) ambiguous account → no send (account_id مفقود في read/draft)
