...
check("QuickSuggestions has onTapGesture", '.onTapGesture { onTap(cmd) }' in qs)
...
check("calendar case routes to runCalendar", 'case .calendar' in vm and 'runCalendar' in vm)
check("reminders path present", 'runReminders' in vm)
...
check("HomeView passes onTap to handleQuickCommand", 'vm.handleQuickCommand(cmd)' in hv)
